...
/*   Updated: 2020/02/26 16:48:28 by sselusa          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
							...
							long double num, int precision)
{
	...
	last_two = (unsigned long)(ft_pow(10, (precision + 2)) * (num - whole));
	last = (unsigned long)(ft_pow(10, (precision + 1)) * (num - whole));
	fract = (unsigned long)(ft_pow(10, (precision)) * (num - whole));
	if (last - (fract * 10) > 5)
		fract++;
	else if (last - (fract * 10) == 5 && last_two - (last * 10) > 0)
		fract++;
	else if (last - (fract * 10) == 5 && last_two - (last * 10) == 0)
		fract += ft_is_even(fract) ? 0 : 1;
	else
		ld->str = NULL;
